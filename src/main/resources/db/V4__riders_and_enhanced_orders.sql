-- Delivery Route Cost Optimizer schema upgrade V4
-- Adds Rider management and enhanced Order lifecycle

-- 1. Create riders table for delivery partner management
...
-- 2. Update delivery_orders table with new columns if they don't exist
ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS assigned_rider_id BIGINT NULL;
ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS weight_kg DECIMAL(10,2) DEFAULT 0;
ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS pickup_slot VARCHAR(30);
ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS picked_at TIMESTAMP NULL;
ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS in_transit_at TIMESTAMP NULL;
ALTER TABLE delivery_orders ADD COLUMN IF NOT EXISTS customer_email VARCHAR(191);

-- 3. Add foreign key for rider assignment if not exists
...
-- 4. Seed dummy riders if none exist
...
-- 5. Seed dummy orders if table is empty
